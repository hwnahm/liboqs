...
extern int pqcrystals_dilithium2_ref_keypair(uint8_t *pk, uint8_t *sk, uint8_t *seed);
...
extern int pqcrystals_dilithium2_avx2_keypair(uint8_t *pk, uint8_t *sk, uint8_t *seed);
...
extern int PQCLEAN_DILITHIUM2_AARCH64_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, uint8_t *seed);
...
OQS_API OQS_STATUS OQS_SIG_dilithium_2_keypair(uint8_t *public_key, uint8_t *secret_key, uint8_t *seed) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2) && OQS_CPU_has_extension(OQS_CPU_EXT_POPCNT)) {
		...
		return (OQS_STATUS) pqcrystals_dilithium2_avx2_keypair(public_key, secret_key, seed);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_dilithium2_ref_keypair(public_key, secret_key, seed);
	}
#endif /* OQS_DIST_BUILD */
...
#endif /* OQS_DIST_BUILD */
		return (OQS_STATUS) PQCLEAN_DILITHIUM2_AARCH64_crypto_sign_keypair(public_key, secret_key, seed);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) pqcrystals_dilithium2_ref_keypair(public_key, secret_key, seed);
	}
