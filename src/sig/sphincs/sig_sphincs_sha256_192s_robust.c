...
extern int PQCLEAN_SPHINCSSHA256192SROBUST_CLEAN_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, uint8_t *seed);
...
extern int PQCLEAN_SPHINCSSHA256192SROBUST_AVX2_crypto_sign_keypair(uint8_t *pk, uint8_t *sk, uint8_t *seed);
...
OQS_API OQS_STATUS OQS_SIG_sphincs_sha256_192s_robust_keypair(uint8_t *public_key, uint8_t *secret_key, uint8_t *seed) {
	...
	if (OQS_CPU_has_extension(OQS_CPU_EXT_AVX2)) {
		...
		return (OQS_STATUS) PQCLEAN_SPHINCSSHA256192SROBUST_AVX2_crypto_sign_keypair(public_key, secret_key, seed);
#if defined(OQS_DIST_BUILD)
	} else {
		return (OQS_STATUS) PQCLEAN_SPHINCSSHA256192SROBUST_CLEAN_crypto_sign_keypair(public_key, secret_key, seed);
	}
#endif /* OQS_DIST_BUILD */
#else
	return (OQS_STATUS) PQCLEAN_SPHINCSSHA256192SROBUST_CLEAN_crypto_sign_keypair(public_key, secret_key, seed);
